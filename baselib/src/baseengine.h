...
public:
    ...
    const QString & xivoUserId() const {
        return m_userid;
    }
    ;
    const QString & getFullId() const {
        return m_xuserid;
    }
    ;
    ...
    bool hasPhone(const QString & xid) {
        return m_anylist.value("phones").contains(xid);
    }
    ;
    bool hasAgent(const QString & xid) {
        return m_anylist.value("agents").contains(xid);
    }
    ;
    bool hasQueue(const QString & xid) {
        return m_anylist.value("queues").contains(xid);
    }
    ;

    QHash<QString, XInfo *> iterover(const QString & listname) {
        return m_anylist.value(listname);
    }
    ;
    ...
    ; //!< Return the channels to any Xlet
    const QHash<QString, QueueMemberInfo *> & queuemembers() const {
        return m_queuemembers;
    }
    ; //!< Return the queue members to any Xlet

    // public operations

    bool isInMeetme() const {
        return m_meetme_membership.size() > 0;
    }
...
private slots:
    ...
    void popupError(const QString & message, const QString & server_address = "", const QString & server_port = "");
