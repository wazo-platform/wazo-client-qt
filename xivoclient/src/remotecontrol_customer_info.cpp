...
void RemoteControl::then_i_see_a_sheet_with_variables_and_values(const QStringList& args)
{
    ...
    foreach (const QVariant &variable, variables.toList()) {
        ...
        const QString &variable_name = variable_map["Variable"].toString();
        const QString &variable_value = variable_map["Value"].toString();
        QLabel * variable_name_label = current_tab->findChild<QLabel*>(QString("label_name_%1").arg(widget_index));
        QLabel * variable_value_label = current_tab->findChild<QLabel*>(QString("label_value_%1").arg(widget_index));
        this->assert(variable_name_label->text() == variable_name);
        this->assert(variable_value_label->text() == variable_value);
        ++widget_index;
    }
}
