...
{
    b_engine->registerTranslation(":/obj/parking_%1");
...
void XletParking::updateParkinglotConfig(const QString & xid)
{
    ...
    if (! m_parkinglots.contains(xid)) {
        ...
        if (w) {
            ...
            connect(w, SIGNAL(itemDoubleClicked(const QString &, const QString &)),
                    this, SLOT(itemDoubleClicked(const QString &, const QString &)));
...
void XletParking::itemDoubleClicked(const QString & ipbxid,
                                    const QString & number)
{
    // qDebug() << Q_FUNC_INFO << number;
    b_engine->actionCall("dial",
                         "user:special:me",
                         QString("exten:%1/%2").arg(ipbxid).arg(number));
}
